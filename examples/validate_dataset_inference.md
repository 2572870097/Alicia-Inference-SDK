...
```bash
cd /home/synria/Alicia-Inference-SDK
...
如果传了 `--temporal-ensemble`，脚本会在 `step` 模式下启用通用时间集成：

- 每一帧都会重新跑一次 `_predict_chunk()`
- 对重叠时间段的未来动作做指数加权融合
- 这个逻辑不只适用于 ACT，也可以直接用于 `pi0` 和 `smolvla`
...
- `--temporal-ensemble`：在 `step` 模式下启用通用时间集成
...
如果报 `ModuleNotFoundError`，通常是环境里缺少脚本依赖，例如：
