...
def _load_action_metadata(dataset: LeRobotDataset) -> tuple[np.ndarray, np.ndarray]:
...
def _plot_episode_curves(
    *,
    ...
    for axis in axes_flat[max(0, action_dim - ncols) : action_dim]:
...
def _run_episode_validation(
    *,
    ...
    if execution_mode == "step" and start_async_inference:
    ...
    for position, dataset_index in enumerate(dataset_indices.tolist(), start=1):
        ...
        instruction = _resolve_instruction(model_type, explicit_instruction, item)
        ...
        if execution_mode == "step":
            predicted = session.step(observation)
        else:
            predicted = session.predict_chunk(observation)
        total_call_ms += (time.perf_counter() - start_time) * 1000.0

        predicted_first = _adapt_prediction_for_dataset(predicted, gripper_mode)[0]
        target = all_actions[int(dataset_index)]
        accumulator.update(predicted_first, target)

        predictions.append(predicted_first)
        targets.append(target)
        frame_indices.append(int(item["frame_index"].item()))
    ...
    episode_index = int(dataset.hf_dataset[int(dataset_indices[0])]["episode_index"].item())
...
def _print_run_header(
    *,
    ...
    print(
        f"Execution mode: requested={requested_execution_mode} resolved={resolved_execution_mode}"
    )
    print(
        "Async inference: "
...
def main() -> int:
    ...
    model_type = args.model_type or _infer_model_type(model_dir)
    ...
    config = PolicyLoadConfig(
        ...
        device=DeviceConfig(device=args.device),
        runtime=RuntimeConfig(
            control_fps=float(dataset.fps),
            enable_async_inference=execution_mode == "step" and args.enable_async_inference,
            temporal_ensemble_coeff=temporal_ensemble_coeff,
        ),
        instruction=args.instruction,
    )
    session = InferenceSession.open(config=config)
    ...
    try:
        ...
        async_inference_runtime_enabled = bool(
            ...
            and initial_status.async_inference_enabled
        )
        ...
        for episode_index in episode_ids:
            ...
            result = _run_episode_validation(
                ...
                start_async_inference=async_inference_runtime_enabled,
            )
